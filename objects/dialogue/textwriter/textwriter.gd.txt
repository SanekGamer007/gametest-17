...
var able_to_end: bool = true
var command_regex = RegEx.new()

@onready var DialogueRichText: RichTextLabel = $VBoxContainer/MarginContainer/RichTextLabel
@onready var SpeakerSprite: Sprite2D = $SpeakerParent/Sprite2D


# Called when the node enters the scene tree for the first time.
func _ready() -> void:
	var callable = func(): (dialogue_end.emit())
...
func _text_resource(textresource: DialogueText) -> void:
	...
	var parsed_data = _extract_commands(final_text)
	DialogueRichText.text = parsed_data["text"]
	var DialogueLength = DialogueRichText.get_total_character_count()
	DialogueRichText.visible_characters = 0
	await _write_text(textresource, DialogueLength, parsed_data["cmds"])
	while true:
...
func _choice_resource(choiceresource: DialogueChoice) -> void:
	...
	$VBoxContainer/button_container.visible = true
	...
	await _write_text(choiceresource, DialogueLength, parsed_data["cmds"])
	...
	for i in range(choiceresource.choice_text.size()): #buttons dont have a visible_characts variable so we have to do whatever this is.
		...
		dialoguebutton.text = ""
		$VBoxContainer/button_container/VFlowContainer.add_child(dialoguebutton)
		buttonarray.append(dialoguebutton)

	var buttonpressed = (func(btn_idx: int):
			for i in $VBoxContainer/button_container/VFlowContainer.get_children():
				i.queue_free()
			$VBoxContainer/button_container.visible = false
...
func _do_action(actionresource: Action) -> void:
	if actionresource is ActionJump:
		...
		request_visibility.emit(actionresource.show_dialogue_box)
...
func _do_tools(tool: Dialogue) -> void:
	request_visibility.emit(tool.show_dialogue_box)
...
func _write_text(resource: DialogueText, DialogueLength: int, commands: Dictionary) -> bool:
	
	if !resource.speaker_img:
		$SpeakerParent.visible = false
		$VBoxContainer/MarginContainer.add_theme_constant_override("margin_left", 8)
		$VBoxContainer/MarginContainer.add_theme_constant_override("margin_right", 8)
	else:
		...
		$VBoxContainer/MarginContainer.add_theme_constant_override("margin_left", 0)
		$VBoxContainer/MarginContainer.add_theme_constant_override("margin_right", 0)

	var clean_text_for_audio = DialogueRichText.get_parsed_text()
	var visible_chars_float: float = 0.0
	var current_speed = resource.text_speed
	...
	while DialogueRichText.visible_characters < DialogueLength:
		...
		var current_char_index = DialogueRichText.visible_characters
		if commands.has(current_char_index):
			var cmds_at_pos = commands[current_char_index]
			for cmd in cmds_at_pos:
				match cmd.type:
					"w":
						await get_tree().create_timer(cmd.val).timeout
					"txtspd":
						current_speed = cmd.val
			commands.erase(current_char_index)
		var delta = get_process_delta_time()
		visible_chars_float += current_speed * delta
...
func _extract_commands(raw_text: String) -> Dictionary:
	var commands: Dictionary = {}
	var clean_text: String = raw_text
	var offset: int = 0
	
	var matches = command_regex.search_all(raw_text)
	
	for m in matches:
		var position = m.get_start() - offset
		var tag_type = m.get_string(1) # "w" или "txtspd"
		var value = m.get_string(2).to_float()
		
		if not commands.has(position):
			commands[position] = []
		commands[position].append({"type": tag_type, "val": value})
		
		offset += m.get_string().length()
	
	clean_text = command_regex.sub(raw_text, "", true)
	
	return {"text": clean_text, "cmds": commands}
#endregion
