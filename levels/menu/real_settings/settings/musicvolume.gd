...
func _on_settingsmanager_settings_change() -> void:
	text = format_text % (str(SettingsManager.user_settings.musicvolume * 100).pad_decimals(0) + "%")
...
func _apply(value: float) -> void:
	SettingsManager.set_musicvolume(SettingsManager.user_settings.musicvolume + value)
